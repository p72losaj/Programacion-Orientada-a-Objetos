...
int Ruleta::deleteJugador(string DNI)
{
	...
	for(i=jugadores_.begin();i!=jugadores_.end();i++)
	{
		...
		{
			jugadores_.erase(i);
			cout<<" Jugador "<<i->getDNI()<<" eliminado"<<endl;
			return 1;
		}
	}
...
void Ruleta::escribeJugadores()
{
	...
	if(fich.is_open())
	{
		for(i=jugadores_.begin();i!=jugadores_.end();i++){
			...
			cout<<" Jugador "<<i->getDNI()<<" añadido al fichero"<<endl;
		}
	fich.close();
	}		
}
...
void Ruleta::leeJugadores()
{
	...
	for(i=jugadores_.begin(); i!=jugadores_.end(); i++){
		...
		cout<<"\tLocalidad: "<<i->getLocalidad()<<endl;
		cout<<"\tProvincia: "<<i->getProvincia()<<endl;
		cout<<"\tPais: "<<i->getPais()<<endl;
		cout<<"\tDinero: "<<i->getDinero()<<endl;
	}
}
...
void Ruleta::getEstadoRuleta(int* nj, int* dm, int* lb, int* db){

	cout<<" Dinero banca: "<<*db<<endl;
	cout<<" Lanzamientos bola: "<<*lb<<endl;
	cout<<" Dinero mesa: "<<*dm<<endl;
	cout<<" Numero de jugadores: "<<*nj<<endl;
